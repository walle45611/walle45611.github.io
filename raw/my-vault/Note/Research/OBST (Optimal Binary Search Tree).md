---
blog: true
blog_title: "OBST (Optimal Binary Search Tree)"
blog_date: 2026-09-24
blog_url: https://blog.walle4561.com/articles/posts/optimal-binary-search-tree/
---
...
## 圖 14.9 的資料（範例）
![[Assets/Note/Research/OBST (Optimal Binary Search Tree)/01-圖 14.9 的資料(範例).png|600]]
$$
...
- **有關於 Empty BST 的考慮**：那麼左樹和右樹就是對稱的說，因為 $j=i-1$ 的話代表數已經是空的了，所以也就代表沒有 $key$，所以一定要有 $\text{dummy key}$ 做支持不然這樣結構會出錯誤
	![[Assets/Note/Research/OBST (Optimal Binary Search Tree)/02-Step 1. Optimal Substructure.png]]![[Assets/Note/Research/OBST (Optimal Binary Search Tree)/03-Step 1. Optimal Substructure.png|350]]

* **結論**：Optimal BST 問題具備最優子結構。

## Step 2. Recursive Subproblem
![[Assets/Note/Research/OBST (Optimal Binary Search Tree)/04-Step 2. Recursive Subproblem.png]]
...
### ALGO
![[Assets/Note/Research/OBST (Optimal Binary Search Tree)/05-ALGO.png]]
- $O(n^3)$ 可以想 $O(n^2) \times O(n)$ 因為 $n \times n$ 格格子，那麼在跑 n 次所以就是 $O(n^3)$ 
- 可以想像 $l$ 是滑動視窗跟 $i$ 配合 $j$ 負責最後的位置
### 計算方式
![[Assets/Note/Research/OBST (Optimal Binary Search Tree)/06-計算方式.png]]