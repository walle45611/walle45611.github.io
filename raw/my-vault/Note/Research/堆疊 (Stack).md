...
blog: true
blog_title: "堆疊 (Stack)"
blog_date: 2024-08-28
blog_url: https://blog.walle4561.com/articles/posts/stack-1/
---
...
2. 若讀到運算元（字母或數字），直接加入輸出序列
    
3. 若讀到左括號 `(`，把它 `push`
    
4. 若讀到右括號 `)`，就從堆疊不斷 `pop` 並加入輸出，直到遇到左括號為止 `(` 然後把左括號丟掉
    
5. 若讀到運算子，則比較它和堆疊頂端運算子的優先權與結合性：
    
    - 當堆頂運算子優先權更高，或優先權相同且屬左結合時，就先彈出堆頂並加入輸出
        
    - 重複上述比較後，再把當前運算子推入堆疊  
        最後輸入掃描結束後，將堆疊中剩下的所有運算子依序彈出並加入輸出。
...
string infixToPostfix(string expr, bool isPrefix){
  stack<char> s;
  string out;
  ...
  for(char c: expr){
    ...
    if(isalnum(c)){
      ...
      while(!s.empty() && (opOrder(s.top()) > opOrder(c)
            || (opOrder(s.top()) == opOrder(c) && !isPrefix))){
        out += s.top(); s.pop();
      }
      s.push(c);
    }
...
string infixToPrefix(string expr) {
    ...
    string revPost = infixToPostfix(expr, true);
...
int main(){
  string infix = "a+b+c";
  string postfix = infixToPostfix(infix,false);
