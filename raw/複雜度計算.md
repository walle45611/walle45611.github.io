...
blog: true
legacy_blog_file: "source/_posts/複雜度計算.md"
---
...
$$
它特別 **適用於** 分治（Divide & Conquer）演算法，因為這類演算法通常會把一個大小為 $n$ 的問題，切成 $a$ 個大小約為 $n/b$ 的子問題，然後再做額外的合併工作 $f(n)$。其中必須有 $b>1$，否則就不是把問題規模縮小；主要比較的是 $n^{\log_b a}$ 和 $f(n)$ 的成長速度。
...
**為何要使用**：原始 Master 定理的 Case 2 只處理下列情況：
$$
f(n)=\Theta\bigl(n^{\log_b a}\bigr)
$$
這種「純多項式」情況，無法涵蓋像 $n \log n$ 這種在臨界階 $n^{\log_b a}$ 上再額外乘上一個 $\log n$ 因子的形式。

若
$$
f(n)=\Theta\bigl(n^{\log_b a}\,\log^{k} n\bigr),\quad k \ge 0,
$$
則
$$
