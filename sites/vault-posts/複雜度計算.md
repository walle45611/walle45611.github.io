...
description: "My vault 資料結構筆記：複雜度計算。"
date: 2025-06-23
blog: true
vault_source: "Note/Research/複雜度計算.md"
---
...
- 下圖為解釋 $n\log_b a$ 由來
	![Master Theorem 解釋1](/vault-assets/bf6daf30b79c6c0cfebb.png)
...
**三種情況（k 版）**
設 $d=\log_b a$。若 $f(n)=\Theta\big(n^{d}\log^{k} n\big)$：
...
2. 把 $f(n)$ 寫成 $n^{d}\cdot(\log n)^{k}$ 找出 $k$
...
是否要「乘樹高」取決於**各層總成本是否近乎常數級**（如 $\Theta(n)$）。  
