...
1. `raw/` 預設是唯讀來源區。`raw/my-vault/` 鏡像原始 My vault，`raw/web-clipper/` 保存剪藏。只有使用者明確要求匯入、同步或重新分類時，才可變更相關原始檔；操作前核對來源與目標，避免覆蓋無關內容。
...
1. 不要修改 `raw/` 既有檔案；只有在使用者明確要求匯入或建立原始來源時，才能新增目標路徑尚不存在的檔案。
