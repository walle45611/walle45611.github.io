...
1. 最終回覆預設應使用 `zh-TW`。
2. 最終回覆禁止使用 Markdown。
3. 禁止輸出 Markdown 標題、粗體清單、表格、巢狀列表、粗體嚴格禁止。
4. 禁止輸出例如 `- **其他相關頁面**`、`- **重點整理**`、`## 標題`、`### 小節`、`1. 清單項目` 這類格式。
5. 需要分段時，使用純文字段落；若任務規則另有要求，可搭配 `【】` 標示。
6. 應適度加入表情符號🔗、1️⃣，等等表情符號，強化可讀性與掃讀性，但不可因此犧牲清楚度。
