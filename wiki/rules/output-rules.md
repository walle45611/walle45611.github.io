...
- 正確格式範例如下
	...
	*1️⃣ OpenClaw 相關內容摘要*
	...
	OpenClaw 被定義為一個 AI agent 的「非 AI」程式框架，其核心作用是在「人類指令」與「LLM 回應」之間建立執行閉環。它並非模型本身，而是透過組裝 system prompt、調用工具、管理記憶檔案、排程與上下文壓縮，讓 LLM 展現出具備目標與長期記憶的助理行為。
	
	*2️⃣ 核心運作機制*
	
	\- *身份與目標注入*：透過將多個本地檔案 \(如 *agent\.md*、*soul\.md*、*memory\.md*\) 拼接成 system prompt，賦予模型人格與任務目標。  
	\- *工具調用 \(Tool Calling\)*：這是 agent 執行任務的核心，透過調用工具 \(特別是 *execute* 類高權限工具\) 來與外部世界互動。  
	\- *上下文工程 \(Context Engineering\)*：利用按需載入 skill、子代理 \(subagent\) 分工、記憶檢索，以及上下文壓縮 \(compaction/pruning\) 來管理有限的 context window。  
	\- *自動化執行*：透過 heartbeat 與 cron job 機制，讓 agent 從被動回應轉變為可定期自動執行任務的狀態。
	
	*3️⃣ 安全與風險防禦*
	
	\- *安全風險*：高權限工具的使用、prompt injection 以及在上下文壓縮過程中可能導致關鍵約束遺失，都是主要的風險點。  
	\- *防禦建議*：應在 harness 層建立硬規則 \(如工具白名單、執行前人工核准\)、將高風險規則寫入長期記憶檔，並在生產環境中使用隔離機器與獨立帳號以降低爆炸半徑。
	
	*4️⃣ 原始連結*
	
	[原始影片連結](https://www.youtube.com/watch?v=2rcJdFuNbZQ&t=1s)
	```
